...
var myAtoi = function(str) {
    ...
    if (!res.length || (res.length === 1 && (res[0] == '-' || res[0] == '+'))) {
        return number
    }
    number = parseInt(res.join(''))
    if (number > max) {
        return max
    } else if (number < min) {
        return min
...
var nextPermutation = function(nums) {
    ...
    //     if (path.length === nums.length && !obj[path]) {
    //         arr.push(path)
    //         obj[path] = 1
    //         return
    //     }
    ...
    //         path += left[i]
    //         combin(path, left.filter((e, index) => index !== i))
    ...
    // debugger
    // arr.sort((a, b) => a - b)
    // let index = arr.indexOf(nums.join(''))
    ...
    //     const char = arr[index].charAt(i)
    //     nums[i] = parseInt(char)
    // }
    let len = nums.length;
    if (len <= 1) return;
...
var rotate = function(nums, k) {
    ...
    arr.map((el, index) => nums[index] = arr[index])
};